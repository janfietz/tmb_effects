...
/*===========================================================================*/
static void DrawCharConf(uint8_t* charConf, int16_t x, int16_t y, systime_t time, struct Effect* next, struct DisplayBuffer* display)
{
    uint8_t column;
    for (column = 0; column < 5; column++)
    {
        ...
        for (line = 0; line < 5; line++)
        {
            ...
            {
                EffectUpdate(next, x + line, y + column, time, display);
            }
        }
    }
}
...
void EffectFont5x5Update(int16_t x, int16_t y, systime_t time, void* effectcfg, void* effectdata, struct Effect* next, struct DisplayBuffer* display)
{
    ...
    if (charConf != NULL)
    {
        DrawCharConf(charConf, x, y, time, next, display);
    }
}
